...
pkgdatadir = $(datadir)/fyrecore
pkgincludedir = $(includedir)/fyrecore
pkglibdir = $(libdir)/fyrecore
pkglibexecdir = $(libexecdir)/fyrecore
...
am__aclocal_m4_deps = $(top_srcdir)/build-aux/m4/ax_boost_base.m4 \
	...
	$(top_srcdir)/build-aux/m4/fyre_find_bdb48.m4 \
	$(top_srcdir)/build-aux/m4/fyre_qt.m4 \
	$(top_srcdir)/build-aux/m4/fyre_subdir_to_include.m4 \
...
CONFIG_HEADER = $(top_builddir)/src/config/fyre-config.h
CONFIG_CLEAN_FILES = libfyreconsensus.pc share/setup.nsi \
	share/qt/Info.plist src/test/buildenv.py \
	qa/pull-tester/run-fyred-for-test.sh \
...
ACLOCAL = ${SHELL} /home/host/testing-master/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
AUTOCONF = ${SHELL} /home/host/testing-master/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /home/host/testing-master/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /home/host/testing-master/build-aux/missing automake-1.15
AWK = mawk
BDB_CPPFLAGS = 
...
MAINT = 
MAKEINFO = ${SHELL} /home/host/testing-master/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
...
OTOOL64 = 
PACKAGE = fyrecore
PACKAGE_BUGREPORT = https://github.com/fyrecrypto/fyre/issues
PACKAGE_NAME = Fyre Core
PACKAGE_STRING = Fyre Core 1.0.0
PACKAGE_TARNAME = fyrecore
PACKAGE_URL = 
...
ZMQ_CFLAGS = 
ZMQ_LIBS = 
abs_builddir = /home/host/testing-master
abs_srcdir = /home/host/testing-master
abs_top_builddir = /home/host/testing-master
abs_top_srcdir = /home/host/testing-master
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /home/host/testing-master/build-aux/install-sh
...
pkgconfig_DATA = libfyreconsensus.pc
FYRED_BIN = $(top_builddir)/src/fyred$(EXEEXT)
FYRE_QT_BIN = $(top_builddir)/src/qt/fyre-qt$(EXEEXT)
FYRE_CLI_BIN = $(top_builddir)/src/fyre-cli$(EXEEXT)
FYRE_WIN_INSTALLER = $(PACKAGE)-$(PACKAGE_VERSION)-win$(WINDOWS_BITS)-setup$(EXEEXT)
OSX_APP = Fyre-Qt.app
OSX_DMG = Fyre-Core.dmg
OSX_BACKGROUND_IMAGE = background.tiff
OSX_DEPLOY_SCRIPT = $(top_srcdir)/contrib/macdeploy/macdeployqtplus
OSX_FANCY_PLIST = $(top_srcdir)/contrib/macdeploy/fancy.plist
OSX_BASE_LPROJ_DIR = $(top_srcdir)/contrib/macdeploy/Base.lproj/InfoPlist.strings
OSX_INSTALLER_ICONS = $(top_srcdir)/src/qt/res/icons/fyre.icns
...
WINDOWS_PACKAGING = $(top_srcdir)/share/pixmaps/fyre.ico \
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info block_test.info \
  leveldb_baseline.info test_fyre_filtered.info total_coverage.info \
  baseline_filtered.info block_test_filtered.info rpc_test.info rpc_test_filtered.info \
  leveldb_baseline_filtered.info test_fyre_coverage.info test_fyre.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/fyre.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/Fyre-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
CLEANFILES = $(OSX_DMG) $(FYRE_WIN_INSTALLER)
all: all-recursive
...
src/config/fyre-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/fyre-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/fyre-config.h
$(top_srcdir)/src/config/fyre-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/fyre-config.h src/config/stamp-h1
libfyreconsensus.pc: $(top_builddir)/config.status $(srcdir)/libfyreconsensus.pc.in
...
src/test/buildenv.py: $(top_builddir)/config.status $(top_srcdir)/src/test/buildenv.py.in
	cd $(top_builddir) && $(SHELL) ./config.status $@
qa/pull-tester/run-fyred-for-test.sh: $(top_builddir)/config.status $(top_srcdir)/qa/pull-tester/run-fyred-for-test.sh.in
	cd $(top_builddir) && $(SHELL) ./config.status $@
...
$(FYRE_WIN_INSTALLER): all-recursive
	$(MKDIR_P) $(top_builddir)/release
	STRIPPROG="$(STRIP)" $(INSTALL_STRIP_PROGRAM) $(FYRED_BIN) $(top_builddir)/release
	STRIPPROG="$(STRIP)" $(INSTALL_STRIP_PROGRAM) $(FYRE_QT_BIN) $(top_builddir)/release
	STRIPPROG="$(STRIP)" $(INSTALL_STRIP_PROGRAM) $(FYRE_CLI_BIN) $(top_builddir)/release
...
$(OSX_APP)/Contents/Resources/fyre.icns: $(OSX_INSTALLER_ICONS)
	$(MKDIR_P) $(@D)
	$(INSTALL_DATA) $< $@

$(OSX_APP)/Contents/MacOS/Fyre-Qt: $(FYRE_QT_BIN)
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/Fyre-Qt

$(OSX_DMG): $(APP_DIST_EXTRAS)
	$(GENISOIMAGE) -no-cache-inodes -D -l -probe -V "Fyre-Core" -no-pad -r -dir-mode 0755 -apple -o $@ dist
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/Fyre-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_fyre.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_fyre -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_fyre_filtered.info: test_fyre.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#block_test.info: test_fyre_filtered.info
#	$(MKDIR_P) qa/tmp
#	-@TIMEOUT=15 qa/pull-tester/run-fyred-for-test.sh $(JAVA) -jar $(JAVA_COMPARISON_TOOL) qa/tmp/compTool $(COMPARISON_TOOL_REORG_TESTS)
#	$(LCOV) -c -d $(abs_builddir)/src --t FyreJBlockTest -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb
...
#rpc_test.info: test_fyre_filtered.info
...
#test_fyre_coverage.info: baseline_filtered_combined.info test_fyre_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_fyre_filtered.info -o $@

#total_coverage.info: baseline_filtered_combined.info test_fyre_filtered.info block_test_filtered.info rpc_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_fyre_filtered.info -a block_test_filtered.info -a rpc_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_fyre.coverage/.dirstamp:  test_fyre_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
#cov: test_fyre.coverage/.dirstamp total.coverage/.dirstamp

# Fyre: disable the test until upstream switches from convoluted comparison tool
# Ref https://github.com/fyre/fyre/issues/4545
#check-local:

.INTERMEDIATE: $(COVERAGE_INFO)

clean-local:
	rm -rf coverage_percent.txt test_fyre.coverage/ total.coverage/ qa/tmp/ cache/ $(OSX_APP)
