...
###   This script attempts to download the signature file SHA256SUMS.asc from fyre.org
...
RCSUBDIR="test/"
BASEDIR="https://fyre.org/bin/"
VERSIONPREFIX="fyre-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'fyre-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Fyre Core binary release signing key installed?"
   fi
