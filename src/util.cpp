...
const char * const FYRE_CONF_FILENAME = "fyre.conf";
const char * const FYRE_PID_FILENAME = "fyred.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "fyre" is a composite category enabling all Fyre-related debug output
            if(ptrCategory->count(string("fyre"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\FyreCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\FyreCore
    // Mac: ~/Library/Application Support/FyreCore
    // Unix: ~/.fyrecore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "FyreCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/FyreCore";
#else
    // Unix
    return pathRet / ".fyrecore";
#endif
#endif
}
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", FYRE_CONF_FILENAME));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty fyre.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override fyre.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", FYRE_PID_FILENAME));
