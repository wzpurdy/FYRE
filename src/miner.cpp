...
CBlockTemplate* CreateNewBlock(const CChainParams& chainparams, const CScript& scriptPubKeyIn)
{
...

    {
        ...
        // NOTE: unlike in fyre, we need to pass PREVIOUS block height here
...
// ***TODO*** ScanHash is not yet used in Fyre
//
...
// ***TODO*** that part changed in fyre, we are using a mix with old one here for now
void static FyreMiner(const CChainParams& chainparams, CConnman& connman)
{
    LogPrintf("FyreMiner -- started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("fyre-miner");
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("FyreMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("FyreMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("FyreMiner -- terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("FyreMiner -- runtime error: %s\n", e.what());
        return;
    }
}
...
void GenerateFyres(bool fGenerate, int nThreads, const CChainParams& chainparams, CConnman& connman)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&FyreMiner, boost::cref(chainparams), boost::ref(connman)));
}
