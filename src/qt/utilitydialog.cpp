    ...
    ui(new Ui::HelpMessageDialog)
{
    ...
    QString version = tr("Fyre Core") + " " + tr("version") + " " + QString::fromStdString(FormatFullVersion());
    ...
    if (helpMode == about)
    {
        setWindowTitle(tr("About Fyre Core"));
        ...
        QString header = tr("Usage:") + "\n" +
            "  fyre-qt [" + tr("command-line options") + "]                     " + "\n";
        ...
        std::string strUsage = HelpMessage(HMM_FYRE_QT);
        ...
        if (showDebug) {
            strUsage += HelpMessageOpt("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", FyreGUI::DEFAULT_UIPLATFORM));
        }
...
All the Fyre in your wallet is comprised of different \"inputs\" which you can think of as separate, discrete coins.<br> \
...
These denominations are 0.01 FYRE, 0.1 FYRE, 1 FYRE and 10 FYRE -- sort of like the paper money you use every day.</li> \
    ...
    QWidget(parent, f)
{
    ...
    layout->addWidget(new QLabel(
        tr("Fyre Core is shutting down...") + "<br /><br />" +
        tr("Do not shut down the computer until this window disappears.")));
    setLayout(layout);
}

QWidget *ShutdownWindow::showShutdownWindow(FyreGUI *window)
{
    if (!window)
        return nullptr;
