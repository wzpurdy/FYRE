...
const std::string FyreGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString FyreGUI::DEFAULT_WALLET = "~Default";

FyreGUI::FyreGUI(const PlatformStyle *platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    clientModel(0),
    walletFrame(0),
    ...
    platformStyle(platformStyle)
{
    ...
    QPixmap pixmapLogo(":/images/light/fyre_logo_horizontal");
...
void FyreGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Fyre address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and fyre: URIs)"));
    ...
    aboutAction = new QAction(tr("&About Fyre"), this);
    aboutAction->setStatusTip(tr("Show information about Fyre"));
    ...
    optionsAction->setStatusTip(tr("Modify configuration options for Fyre"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Fyre addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Fyre addresses"));
    ...
    openAction->setStatusTip(tr("Open a fyre: URI or payment request"));

    showHelpMessageAction = new QAction(QApplication::style()->standardIcon(QStyle::SP_MessageBoxInformation), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the Fyre help message to get a list with possible Fyre command-line options"));
...
void FyreGUI::createToolBars(QWidget* statusbar)
{
    if(walletFrame)
    {
...
void FyreGUI::setClientModel(ClientModel *clientModel)
{
...
#ifdef ENABLE_WALLET
bool FyreGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool FyreGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void FyreGUI::setWalletActionsEnabled(bool enabled)
{
...
void FyreGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
    trayIcon = new QSystemTrayIcon(this);
    QString toolTip = tr("Fyre client") + " " + networkStyle->getTitleAddText();
...
void FyreGUI::createIconMenu(QMenu *pmenu)
{
...
#ifndef Q_OS_MAC
void FyreGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void FyreGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void FyreGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void FyreGUI::gotoSettingsPage() 
{
    settingsAction->setChecked(true);
    if(walletFrame) walletFrame->gotoSettingsPage();
}

void FyreGUI::gotoToolsPage()
{
    toolsAction->setChecked(true);
    if(toolsAction) walletFrame->gotoToolsPage();
}

void FyreGUI::gotoMasternodePage()
{
    QSettings settings;
    if (settings.value("fShowMasternodesTab").toBool()) {
...
void FyreGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void FyreGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void FyreGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void FyreGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
#endif // ENABLE_WALLET

void FyreGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        labelConnectionsIcon->setToolTip(tr("%n active connection(s) to Fyre network", "", count));
    } else {
...
void FyreGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void FyreGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void FyreGUI::updateHeadersSyncProgressLabel()
{
...
void FyreGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void FyreGUI::setAdditionalDataSyncProgress(double nSyncProgress)
{
    if(!clientModel)
        return;
...
void FyreGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Fyre"); // default title
...
void FyreGUI::changeEvent(QEvent *e)
{
...
void FyreGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void FyreGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void FyreGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(FyreUnits::formatWithUnit(unit, amount, true)) +
                  tr("Type: %1\n").arg(type);
    if (!label.isEmpty())
        msg += tr("Label: %1\n").arg(label);
...
void FyreGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void FyreGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool FyreGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool FyreGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void FyreGUI::setHDStatus(int hdEnabled)
{
    QString theme = GUIUtil::getThemeName();
...
void FyreGUI::setEncryptionStatus(int status)
{
    QString theme = GUIUtil::getThemeName();
    switch(status)
    {
...
void FyreGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void FyreGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void FyreGUI::detectShutdown()
{
...
void FyreGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void FyreGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void FyreGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(FyreGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void FyreGUI::subscribeToCoreSignals()
{
...
void FyreGUI::unsubscribeFromCoreSignals()
{
...
void FyreGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void FyreGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<FyreUnits::Unit> units = FyreUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    Q_FOREACH (const FyreUnits::Unit unit, units)
    {
        max_width = qMax(max_width, fm.width(FyreUnits::name(unit)));
    }
...
{
    menu = new QMenu(this);
    Q_FOREACH(FyreUnits::Unit u, FyreUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(FyreUnits::name(u)), this);
...
{
    setText(FyreUnits::name(newUnits));
}
